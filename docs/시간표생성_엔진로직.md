...
| HC-12 | `BasicHcs.AddHc12_Lunch` | 선택한 점심 교시 금지; 요일별 모드는 4·5교시 중 정확히 하나를 비움 |
...
| HC-19 | `BlockHcs.AddHc19_Len2StartPeriods` | 점심을 제외한 연속 구간에서 2시간 블록 시작 교시 계산 |
...
| 고정 과목이 선택한 점심 교시에 있거나, 요일별 모드에서 같은 날 4·5교시가 모두 고정됨 | `fixed time conflict` |
