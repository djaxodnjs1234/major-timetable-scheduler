...
중요한 점은 솔버가 직접 DB를 수정하지 않는다는 것입니다. 솔버는 `WorkspaceService.Snapshot()`으로 받은 입력만 보고 계산하며, 결과는 `SolutionAssignment` 목록으로 반환합니다.
...
`WorkspaceService.Snapshot()`은 저장된 과목을 `DomainHelpers.ExpandSections()`로 분반 확장한 뒤 솔버에 넘깁니다. 예를 들어 `Section = 3`인 과목은 `과목-01`, `과목-02`, `과목-03`처럼 개별 분반으로 계산됩니다.
...
| `BlockHcs` | HC-18 | 블록 페어 요일 차이 최대 2 |
| `BlockHcs` | HC-19 | 2시간 블록 시작 교시 제한 |
| `BlockHcs` | HC-20 | 같은 과목 블록들은 다른 요일 배치 |
| `BlockHcs` | HC-21 | 교수의 자동 배정 과목은 같은 강의실 후보 안에서 일관 배정 |
...
| SC-03 | 블록 2개 이상 과목 중 블록 요일 차이가 1 이하인 과목 수 | 조건 만족 과목 수 / 대상 과목 수 |
...
| 값 | 의미 |
|---|---|
| `Sc01SlackAbs = 0` | SC-01 측정값 그대로 강제 |
