...
| `BlockHcs` | HC-22 | 자동 배정 과목의 모든 블록·분반을 하나의 공통 강의실에 배정 |
...
| 과목에 `FixedRooms` 없음 | 과목 불가 강의실과 교수 불가 강의실을 제외한 강의실 중 자동 배정 |

따라서 과목이 방을 명시하면 교수의 불가 강의실 조건보다 과목 설정이 우선합니다.
...
`ConflictDetector`는 강의실 중복, 교수 중복, 점심 배치, 분반 중복, 학년 중복, 고정 시간/방 위반, 교수 불가 강의실 위반 등을 검사합니다. 현재 수동 편집은 “편집 후 재솔버”가 아니라 “편집한 결과를 검증하고 저장”하는 흐름입니다.
