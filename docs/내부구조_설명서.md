...
| `BlockHcs` | HC-21 | 삭제됨: 교수 불가 강의실 기능 미사용 |
...
| 과목에 `FixedRooms` 없음 | 과목 불가 강의실을 제외한 강의실 중 자동 배정 |

따라서 과목이 방을 명시하면 과목 설정이 우선합니다.
...
`ConflictDetector`는 강의실 중복, 교수 중복, 점심 배치, 분반 중복, 학년 중복, 고정 시간/방 위반 등을 검사합니다. 현재 수동 편집은 “편집 후 재솔버”가 아니라 “편집한 결과를 검증하고 저장”하는 흐름입니다.
